...
class TestExcludeService(TransactionCase):
    ...
    def test_so_line_is_delivery(self):
        ...
        """
        self.sale.order_line.invalidate_recordset()
        line1 = self.sale.order_line.filtered(
            lambda line: line.product_id == self.product1
        )
        line2 = self.sale.order_line.filtered(
            lambda line: line.product_id == self.service1
        )
        line3 = self.sale.order_line.filtered(
            lambda line: line.product_id == self.service_affect_delivery_date
        )
        self.assertFalse(line1._is_delivery())
        self.assertTrue(line2._is_delivery())
        self.assertFalse(line3._is_delivery())
