...
class DeliveryCarrier(models.Model):
    ...
    def rate_shipment(self, order):
        ...
        # Prevent ``AccessError`` if needed: by default, module ``delivery`` only allows
        # Sales / Administrator users to have ``write`` access on ``delivery.carrier``,
        # but lots of modules use this method, even for automations on ``sale.order``
        # creation/updates (eg: module ``delivery_auto_refresh`` or OCA/sale-workflow
        # ``sale_order_carrier_auto_assign``); therefore, if necessary, we go ``sudo()``
        # before updating/reverting the delivery type on the carrier
        try:
            self.check_access("write")
            carrier = self
        except AccessError:
            _logger.info(f"Get shipment rates in sudo() for user {self.env.uid}")
            carrier = self.sudo()
        delivery_type = carrier.delivery_type
        carrier.delivery_type = "pricelist"
        result = super(DeliveryCarrier, carrier).rate_shipment(order)
